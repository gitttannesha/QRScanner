...
// ─────────────────────────────────────────────
router.get("/lab-locations",(req, res) => {
  pool.query(
...
pool2.query(
  `SELECT * From \`${table}\` WHERE id = ?`, [id], (err, rows) => {
    if (err)                         return res.status(500).json({ success: false, error: "Database error" });
    if (!rows || rows.length === 0) return res.status(404).json({ success: false, message: "Item not found" });
    const consumable = rows[0];
    const addedBy    = consumable.added_by;

    // Fetch name from login table in pool (slotbooking db)
    pool.query(`SELECT fname, lname FROM login WHERE memberid = ?`, [addedBy], (err2, loginRows) => {
      if (err2 || !loginRows || loginRows.length === 0) {
        consumable.added_by_name = "N/A";
      } else {
        consumable.added_by_name = `${loginRows[0].fname} ${loginRows[0].lname}`;
      }
...
// ─────────────────────────────────────────────
router.get("/consumable-stock/:id",(req, res) => {
  const id    = req.params.id;
  const table = req.query.table;
...
router.get("/consumable-members", (req, res) => {
  const search = req.query.search || "";
   console.log("Search received:", search);
   console.log("Search length:", search.trim().length);
  ...
  pool.query(
    `SELECT memberid, fname, lname FROM login WHERE position != 'Faculty' AND STR_TO_DATE(expiry_date, '%m/%d/%Y') > CURDATE()  AND  (fname LIKE ?) order by fname;`,
    [`${search}%`],
    (err, rows) => {
      if (err) return res.status(500).json({ success: false, error: "Database error" });
      res.json({ success: true, data: rows });
...
router.get("/pending-stock/:consumable_id", (req, res) => {
