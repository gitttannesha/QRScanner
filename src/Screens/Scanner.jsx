...
const ScannerScreen = ({ navigation }) => {
  ...
  const handleBarCodeScanned = async ({ data }) => {
    ...
    console.log("Scanned ID:", cleanData);

    try {
      // ── Chemical ──
      if (cleanData.startsWith("CH_")) {
        const chemicalId = cleanData.replace("CH_", "");
        const res = await apiCall('GET' ,`/chemical/${chemicalId}`);
      ...
      // ── One-Time Consumable ──
      } else if (cleanData.startsWith("CON_OT_")) {
        const itemId = cleanData.replace("CON_OT_", "");
        const res = await apiCall('GET' ,`/consumable/${itemId}?table=one_time_master_new`);
        const dataJson = res.data;
        if (dataJson.success) {
      ...
      // ── Reusable ──
      } else if (cleanData.startsWith("CON_RE_")) {
        const itemId =cleanData.replace("CON_RE_", "");
        const res = await apiCall('GET', `/consumable/${itemId}?table=reusables_master_new`);
        const dataJson = res.data;
        if (dataJson.success) {
          navigation.navigate("ConsumableDetails", {
      ...
      // ── Unknown ──
    // ── Spare Part ──
      } else if (cleanData.startsWith("SP_")) {
        const itemId = cleanData.replace("SP_", "");
        const res = await apiCall('GET' , `/sparepart/${itemId}`);
        const dataJson = res.data;
        if (dataJson.success) {
