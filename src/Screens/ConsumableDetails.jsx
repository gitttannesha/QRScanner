...
// 2. onRefresh function (right below fetchCurrentStock)
const onRefresh = async () => {
  ...
  useEffect(() => {
    const fetchLabs = async () => {
      try {
        const response = await apiCall('GET',`/lab-locations`);
        if (response.data.success) {
          setLabLocations(
...
const filteredMembers = members.filter(m =>
    m.fname.toLowerCase().startsWith(memberSearch.toLowerCase()) ||
    m.lname.toLowerCase().startsWith(memberSearch.toLowerCase())
);
...
const fetchPendingStock = async (member) => {
  ...
  try {
    const response = await apiCall('GET',
      `/pending-stock/${consumable.id}?table=${table}&member_id=${member.memberid}`
    );
 ...
 const validateAmount = () => {
  ...
  const handleAddStock = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        Alert.alert("Success", `Stock updated! New stock: ${Math.floor(response.data.new_stock)}`);
  ...
  const handleIssue = async () => {
    ...
    setIsUpdating(true);
    try {
      const response = await apiCall('POST' , `/issue-consumable`, {
        consumable_id: consumable.id,
        table,
        amount_to_issue: parseFloat(amount),
        issued_to: selectedMember.memberid,   // userid = person it's issued to
        lab_location_id: selectedLocation ? selectedLocation.id : null,
        comment: comment.trim(),
        flag: 0
      });
      if (response.data.success) {
        Alert.alert("Issued", `Remaining stock: ${Math.floor(response.data.new_stock)}`);
        setCurrentStock(response.data.new_stock);
        resetForm();
      } else {
 ...
 const handleReturn = async () => {
  ...
  if (pendingStock !== null && parseFloat(amount) > pendingStock) {
    ...
    try {
      ...
      if (response.data.success) {
        Alert.alert("Returned", `Updated stock: ${Math.floor(response.data.new_stock)}`);
        setCurrentStock(response.data.new_stock);
        resetForm();
      } else {
        Alert.alert("Failed", response.data.message);
          ...
          {showMore && (
            <>
              ...
              <View style={styles.detailRow}>
                ...
                <Text style={styles.value}>{consumable.added_by_name || "N/A"}</Text>
              </View>
              <View style={styles.detailRow}>
                <Text style={styles.label}>Added On</Text>
                <Text style={styles.value}>{formatDate(consumable.timestamp) || "N/A"}</Text>
              </View>
              <View style={styles.detailRow}>
                <Text style={styles.label}>Last Updated</Text>
                <Text style={styles.value}>{formatDate(consumable.last_updated_timestamp) || "N/A"}</Text>
              </View>
            </>
          )}
        </View>
